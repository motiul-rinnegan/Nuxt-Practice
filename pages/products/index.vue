...
<div class="flex flex-col p-2 py-6 m-h-screen">
  <div class="bg-white items-center justify-between w-full flex rounded-full shadow-lg p-2 mb-5 sticky" style="top: 5px">
      <input v-model="search" name="search"  autocomplete="off" @focus="showDiv=true" class="font-bold uppercase rounded-full w-96 py-4 pl-4 text-gray-700 bg-gray-100 leading-tight focus:outline-none focus:shadow-outline lg:text-sm text-xs" type="text" placeholder="Search"> 
      ...
      <div v-if="fetchProducts.length" class="z-40">
        <ul v-show="showDiv" v-for="filterproduct in fetchProducts" v-bind:key="filterproduct.title" @click="setState(filterproduct)" class="font-bold uppercase text-gray-700  w-62 bg-white-800 text-black">
  ...
  <div class="flex flex-col gap-4 lg:p-4 p-2  rounde-lg m-2">
    ...
    <!--Card 1-->
    <div class="rounded overflow-hidden shadow-lg" v-for="item in fetchProducts">
      <div class="px-6 py-4">
...
const {data:fetch}=await useFetch(`https://fakestoreapi.com/products`);
